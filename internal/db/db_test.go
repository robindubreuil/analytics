...
func TestMigrate(t *testing.T) {
	...
	if version != currentSchemaVersion {
		t.Errorf("Expected migration version %d, got %d", currentSchemaVersion, version)
	}
}
...
func TestMaxPathSepIndex(t *testing.T) {
	tests := []struct {
		input    string
		expected int
	}{
		...
		{"/file.db", 0},
		{"C:\\path\\to\\file.db", 10},
		{"/a/b/c/d/e/file.db", 10},
	}

	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			result := maxPathSepIndex(tt.input)
			if result != tt.expected {
				t.Errorf("maxPathSepIndex(%q) = %d, want %d", tt.input, result, tt.expected)
			}
		})
	}
}
